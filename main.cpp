...
// ---------------------------------------------------------------------------
    ...
    // Inside the Cube class, add the following method to print the state of a face:
    void print_face(const Face& face) const {
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 3; ++j) {
                std::cout << face[i][j] << " ";
            }
    ...
    // Add a method to print the state of all the faces:
    void print_cube() const {
        for (const auto& face_pair : faces) {
            std::cout << face_pair.first << ":" << std::endl;
            print_face(face_pair.second);
            std::cout << "-------------" << std::endl;
        }
    }


    // Define all the move functions here
